...
class CreatePayLogTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('pay_log', function (Blueprint $table) {
            ...
            $table->string('bank_type',32)->comment('类型');
            $table->string('cash_fee',32)->comment('金额');
            $table->string('fee_type',32)->comment('支付类型');
            $table->string('id')->comment('订单id');
            $table->string('is_subscribe',32)->comment('***');
            $table->string('mch_id',32)->comment('微信支付分配的商户号');
            ...
            $table->string('result_code',32)->comment('返回码');
            $table->string('return_code',32)->comment('返回参数');
